...
<section class="page_title">
	...
	@endswitch">
		<div class="calendar">
			<span class="day">{{ $logo->date?->isoFormat('DD') }}</span>
			<br>
			<span class="month">{{ $logo->date?->isoFormat('MMM') }}</span>
		</div>
		<h1>{{ $logo->title }}</h1>
...
<section class="show">
	...
	<aside class="info">
		<a href="{{ route('programme', ['commune' => $logo->commune]) }}" class="commune"><div class="dot" style="background-color: 
			@switch($logo->commune)
			@case(1150) #fca91c @break
			@case(1160) #ec008d @break
			@case(1170) #6fcee5 @break
			@case(1200) #acc337 @break
			@endswitch"></div><span>{{ $logo->commune }}</span>| 
			@switch($logo->commune)
		...
		<h3>
			...
			{{ $filters->firstWhere('id', $logo->filter)?->name }}
		</h3>
		@if ($logo->schedule)
			<h3>Horaire</h3>
			<p>{{ $logo->schedule }}</p>
...
<section id="form">
	...
	@if (today()->format('y-m-d') >= date('22-02-28'))
		...
		@if ($logo->participants->count() < $logo->places)
			@livewire('register', ['logo' => $logo])
		@else
